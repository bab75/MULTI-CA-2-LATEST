...
def make_prediction(pipeline, X):
    ...
    try:
        ...
        # DIRECT CA DETERMINATION: If we have attendance_percentage, use it as the ground truth
        # This handles the case where the model predicts something else but we want to respect
        # the actual attendance percentage threshold for determining CA status
        if has_attendance:
            # Override predictions based on the 90% attendance threshold
            ca_threshold = 90.0  # Hardcoded threshold from config
            
            # Create new prediction array with CA/No-CA strings for clearer output
            ...
            print(f"Using direct attendance threshold of {ca_threshold}% to determine CA status")
            print(f"Found {sum(1 for p in final_predictions if p == 'CA')} CA students based on attendance threshold")
            
            return final_predictions, probabilities
        
        # For model-based predictions, convert numeric/boolean to readable CA/No-CA format
