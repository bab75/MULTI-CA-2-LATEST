    ...
    if st.button("Predict CA Risk", key="run_prediction_button"):
        with st.spinner("Running predictions..."):
            try:
                                ...
                                if severity in ca_students['severity'].values:
                                    ...
                                    if severity == 'Severe':
                                        ...
                                        <div style="background-color: #FEE2E2; padding: 15px; border-radius: 5px; margin-bottom: 10px; border-left: 4px solid #DC2626;">
                                            ...
                                            <p><em>Students in this category will likely need full-year intervention to improve habits, with reduced expectation of achieving Non-CA status this year.</em></p>
                                        </div>
                                        """, unsafe_allow_html=True)
                                    elif severity == 'High':
                                        st.markdown("""
                ...
                if 'attendance_percentage' in prediction_data.columns:
                    # Create a direct CA status based purely on attendance threshold
                    attendance_threshold = 90.0
                    attendance_ca_status = ['CA' if att <= attendance_threshold else 'No-CA' 
                                          for att in prediction_data['attendance_percentage']]
                    
                    attendance_ca_count = sum(1 for status in attendance_ca_status if status == 'CA')
                    
                    # Display comparison
                    st.markdown("### Attendance vs Prediction Comparison")
                    comparison_cols = st.columns(2)
                    
                    with comparison_cols[0]:
                        st.markdown(f"""
                        <div style="background-color: #FEF2F2; padding: 15px; border-radius: 5px; text-align: center;">
                            <h4 style="margin-top: 0;">Direct Attendance-Based</h4>
                            <h2 style="color: #DC2626; font-size: 2.5rem; margin: 10px 0;">{attendance_ca_count}</h2>
                            <p>CA students (attendance ≤ 90%)</p>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    with comparison_cols[1]:
                        st.markdown(f"""
                        <div style="background-color: #ECFDF5; padding: 15px; border-radius: 5px; text-align: center;">
                            <h4 style="margin-top: 0;">Model Prediction</h4>
                            <h2 style="color: #059669; font-size: 2.5rem; margin: 10px 0;">{ca_count}</h2>
                            <p>CA students (predicted by model)</p>
                        </div>
                        """, unsafe_allow_html=True)
